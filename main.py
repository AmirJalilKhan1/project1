...
def embed_image(cover_img_path, secret_img_path):
    ...
    for y in range(secret_img_height):
        for x in range(secret_img_width):
            ...
            cover_r, cover_g, cover_b = int_to_bin(cover_pixel[0]), int_to_bin(cover_pixel[1]), int_to_bin(
                cover_pixel[2])
            secret_r, secret_g, secret_b = int_to_bin(secret_pixel[0]), int_to_bin(secret_pixel[1]), int_to_bin(
                secret_pixel[2])
            merged_r, merged_g, merged_b = merge_bits(cover_r, secret_r), merge_bits(cover_g, secret_g), merge_bits(
                cover_b, secret_b)
...
def extract_image(stego_img_path):
    ...
    for y in range(secret_img_height):
        for x in range(secret_img_width):
            ...
            stego_r, stego_g, stego_b = int_to_bin(stego_pixel[0]), int_to_bin(stego_pixel[1]), int_to_bin(
                stego_pixel[2])
